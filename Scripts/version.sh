#!/usr/bin/env bash
# Shared version + checksum helpers for AdlerIt release tooling.

# Print the version declared in Cargo.toml, prefixed with "v" (e.g. v0.1.0).
adlerit_version() {
  local root_dir="$1"
  printf 'v%s\n' "$(awk -F '"' '/^version = / { print $2; exit }' "$root_dir/Cargo.toml")"
}

# Strip the leading "v" from a tag/version (v0.1.0 -> 0.1.0).
adlerit_package_version() {
  local version="$1"
  printf '%s\n' "${version#v}"
}

# Write a "<sha256>  <filename>" checksum file next to an artifact.
adlerit_write_sha256_checksum() {
